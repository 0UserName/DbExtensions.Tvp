...
namespace DbExtensions.Tvp.Metadata
{
    public static class MetadataStorage
    {
        ...
        {
            return Sort(_storage.TryRemove(MetadataProvider<TRow>.Get<TableMetadataAttribute>().Name, out IColumnInternalMetadata[] metadata) ? metadata : MetadataProvider<TRow>.ExtractColumns());
        }
